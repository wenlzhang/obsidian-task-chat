...
export class TaskPropertyService {
    ...
    private static inferStatusOrderFromPattern(displayName: string): number {
        ...
        if (lower.includes("progress") || lower.includes("working") || lower.includes("active") || lower.includes("进行") || lower.includes("pågående")) {
            return 2;
        }

        // Important/urgent (high priority even if not active)
        if (lower.includes("important") || lower.includes("urgent") || lower.includes("critical") || lower.includes("重要") || lower.includes("viktig")) {
            return 3;
        }

        // Waiting/blocked (medium priority)
        if (lower.includes("wait") || lower.includes("block") || lower.includes("pend") || lower.includes("等待") || lower.includes("väntar")) {
            return 4;
        }

        // Bookmarked (medium-low priority)
        if (lower.includes("bookmark") || lower.includes("mark") || lower.includes("star") || lower.includes("书签") || lower.includes("bokmärke")) {
            return 5;
        }

        // Finished work (lower priority)
        if (lower.includes("complete") || lower.includes("done") || lower.includes("finish") || lower.includes("完成") || lower.includes("klar")) {
            return 6;
        }
        if (lower.includes("cancel") || lower.includes("abandon") || lower.includes("取消") || lower.includes("avbruten")) {
            return 7;
        }
    ...
    static inferStatusDescription(displayName: string): string {
        ...
        if (lower.includes("bookmark") || lower.includes("mark") || lower.includes("star") || lower.includes("flag")) {
            return "Bookmarked or marked tasks for later review";
        }
        if (lower.includes("wait") || lower.includes("block") || lower.includes("hold") || lower.includes("pending")) {
    ...
    static inferStatusTerms(
        ...
        if (lower.includes("cancel") || lower.includes("abandon") || lower.includes("drop") || lower.includes("取消") || lower.includes("avbruten")) {
        ...
        if (lower.includes("other") || lower.includes("其他") || lower.includes("övrig")) {
    ...
    static formatDate(date: any, format?: string): string | undefined {
        ...
        try {
            ...
            if (date && typeof date === "object" && typeof date.toString === "function") {
    ...
    static convertDateFilterToRange(dateFilter: string): DateRange | null {
        ...
        switch (dateFilter) {
            ...
            case "next-week": {
                const nextWeekStart = today.clone().add(7, "days").startOf("day");
            ...
            default:
                ...
                const relativeRange = this.parseRelativeDateRange(dateFilter, today);
    ...
    static parseRelativeDateRange(
        ...
        const durationPattern = /^(\d+)\s*(d|day|days|w|wk|wks|week|weeks|mo|month|months|yr|yrs|year|years)$/;
        const durationMatch = lowerFilter.match(durationPattern);
        if (durationMatch) {
            const amount = parseInt(durationMatch[1]);
            const unit = durationMatch[2];
            const momentUnit = unit.startsWith("d") ? "days"
                : unit.startsWith("w") ? "weeks"
                : unit.startsWith("mo") ? "months"
                : unit.startsWith("yr") ? "years"
                : "days";
            
            const futureDate = today.clone().add(amount, momentUnit as moment.unitOfTime.DurationConstructor);
            return {
                start: today.format("YYYY-MM-DD"),
        ...
        const todoistMatch = lowerFilter.match(/^([+-])?(\d+)\s+(day|days|week|weeks)$/);
        if (todoistMatch) {
        ...
        if (agoMatch) {
            ...
            const unit = agoMatch[2].startsWith("week") ? "weeks"
                ...
                : agoMatch[2].startsWith("year") ? "years"
                : "days";
            const pastDate = today.clone().subtract(amount, unit);
            return {
                start: pastDate.format("YYYY-MM-DD"),
        ...
        const withinMatch = lowerFilter.match(/within\s+(\d+)\s+(day|days|week|weeks|month|months)/);
        if (withinMatch) {
            const amount = parseInt(withinMatch[1]);
            const unit = withinMatch[2].startsWith("week") ? "weeks"
                : withinMatch[2].startsWith("month") ? "months"
                : "days";
            const futureDate = today.clone().add(amount, unit);
            return {
                start: today.format("YYYY-MM-DD"),
        ...
        const nextMatch = lowerFilter.match(/next\s+(\d+)\s+(day|days|week|weeks|month|months|year|years)/);
        if (nextMatch) {
            const amount = parseInt(nextMatch[1]);
            const unit = nextMatch[2].startsWith("week") ? "weeks"
                : nextMatch[2].startsWith("month") ? "months"
                : nextMatch[2].startsWith("year") ? "years"
                : "days";
            const futureDate = today.clone().add(amount, unit);
            return {
                start: today.format("YYYY-MM-DD"),
        ...
        const lastMatch = lowerFilter.match(/last\s+(\d+)\s+(day|days|week|weeks|month|months)/);
        if (lastMatch) {
            const amount = parseInt(lastMatch[1]);
            const unit = lastMatch[2].startsWith("week") ? "weeks"
                : lastMatch[2].startsWith("month") ? "months"
                : "days";
            const pastDate = today.clone().subtract(amount, unit);
            return {
                start: pastDate.format("YYYY-MM-DD"),
    ...
    static filterByDueDate(tasks: Task[], filter: string): Task[] {
        ...
        if (filter === "any") {
            return tasks.filter((task) => task.dueDate !== undefined && task.dueDate !== null && task.dueDate !== "");
        }
